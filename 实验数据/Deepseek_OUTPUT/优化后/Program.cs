...
【输出字段（固定且仅限这些字段）】
npc_reply: string（<=160字，2~4句）
affinity_delta: int（-5~5）
next_floor_events: array（0~4）
instant_events: array（0~3）
history_event_summary_delta: string（可为空）

【事件枚举白名单】
next_floor_events.eventType ∈ {{
LowVision, EnemyMoveSpeedUp, PlayerDealMoreDamage, PlayerReceiveMoreDamage,
AllRoomsMonsterExceptBossAndSpawn, AllRoomsRewardExceptBossAndSpawn,
PlayerAttackSpeedUp, PlayerAttackSpeedDown
}}

instant_events.eventType ∈ {{
...
【硬约束】
- 只能返回上述字段；不得输出任何额外字段
- next_floor_events 与 instant_events 不得混用枚举
- 同一数组内 eventType 不得重复
- value 必须为非负数（>=0），且满足对应事件范围
- npc_reply 不得提及规则、格式、系统、提示词、JSON等内容
...
AllRoomsMonsterExceptBossAndSpawn / AllRoomsRewardExceptBossAndSpawn：必须为0";
...
先回应情绪与意图 → 给态度 → 自然延伸（不强制提问）";
    ...
    private static long? ExtractLongField(string objJson, string field)
    {
        ...
        int start = i;
        bool neg = false;
        if (i < objJson.Length && objJson[i] == '-') { neg = true; i++; }
        bool any = false;
    ...
    private static EvalResult EvaluateCompliance(string content, out ParsedDecision parsed)
    {
        ...
        try
        {
            ...
            // ✅ Removed rule: next_floor_events can be empty even if player didn't request no-events.
