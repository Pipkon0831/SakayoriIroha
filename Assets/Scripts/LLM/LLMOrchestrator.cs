...
"
        ...
        return
$@"你是游戏NPC【{npcName}】。你使用中文，并且只能输出一个合法的 JSON 对象，禁止输出任何 JSON 之外的文字。
...
2) 必须包含态度/情绪/评价，并抛出一个具体话题引导玩家回应。
    ...
    private bool TryMakeLayerEvent(EventJson e, out LayerEvent layerEvent, bool isNextFloor)
    {
        ...
        switch (type)
        {
            ...
            case LayerEventType.WeaponBulletSizeUp:
                // 允许 0（走默认1.2），或 1.05~2.0
                v = Mathf.Clamp(v, 0f, 2.0f);
                break;

            case LayerEventType.WeaponExplosionOnHit:
                // 半径：允许 0（走默认1.5），或 0.8~3.0
                v = Mathf.Clamp(v, 0f, 3.0f);
                break;
        }
